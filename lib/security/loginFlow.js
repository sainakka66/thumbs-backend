...
async function finalizeLogin(req, res, user, { skipDeviceCheck = false } = {}) {
  ...
  if (needsMfa || needsDevice) {
    ...
    const sendMfaEmail = needsMfa && mfa.methods.includes('email');
    ...
    if (dualEmailChallenge) {
      ...
      emailMasked = deviceSend.emailMasked;
      ...
      emailMasked = mfaSend.emailMasked;
    }

    const defaultMethod = needsDevice ? 'device' : mfa.methods[0] || 'email';
    ...
    return res.json({
      ...
      message: needsDevice
        ...
        : 'Multi-factor authentication required. Check your verified email for the code.',
    });
  }
